...
# -------------------- CASH-OUT INTELLIGENCE --------------------
    ...
    # Cash-out forecasting must use only cities that actually exist in the
    # ATM location reference dataset. The main Case Analysis city list can
    # still contain every banking-data city; this dropdown is ATM-backed only.
    atm_city_col = first_existing(atm_df, ["city", "City", "atm_city", "ATM City"])
    if atm_city_col:
        city_options = sorted(
            atm_df[atm_city_col].dropna().astype(str).str.strip().unique().tolist()
        )
    else:
        city_options = ["Bengaluru"]
