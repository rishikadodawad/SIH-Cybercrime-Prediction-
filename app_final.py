...
def build_cashout_forecast(selected_city):
    """Rank candidate ATMs in a complaint city using the Colab's
    historical-withdrawal (70%) + mule-activity (30%) scoring idea.
    This is a rule/score-based forecast, not a separate ML model.
    """
    city_col_atm = first_existing(atm_df, ["City", "city", "atm_city", "ATM City"])
    atm_id_col = first_existing(atm_df, ["ATM_ID", "atm_id", "ATM ID", "id"])
    atm_loc_col = first_existing(
        atm_df,
        ["ATM_Location", "atm_location", "ATM Location", "location", "Location"]
    )
    atm_area_col = first_existing(
        atm_df,
        ["Area", "area", "atm_area", "ATM Area"]
    )

    if not city_col_atm or not atm_loc_col:
        return None, "ATM dataset needs City and ATM Location columns."

    candidates = atm_df[
        atm_df[city_col_atm].astype(str).str.strip().str.lower()
        == str(selected_city).strip().lower()
    ].copy()

    if candidates.empty:
        # Fallback for datasets where city is embedded in text.
        candidates = atm_df[
            atm_df[city_col_atm].astype(str).str.contains(
                str(selected_city), case=False, na=False
            )
        ].copy()
    ...
    withdrawal_location_col = first_existing(
        bank_df, ["withdrawal_location", "Withdrawal Location"]
    )
    withdrawal_area_col = first_existing(
        bank_df, ["withdrawal_area", "Withdrawal Area"]
    )

    history = bank_df.copy()
    if withdrawal_col:
        history = history[history[withdrawal_col].apply(normalize_bool)].copy()

    def historical_count(row):
        if history.empty:
            return 0

        loc = str(row[atm_loc_col]).strip().lower()
        area = str(row[atm_area_col]).strip().lower() if atm_area_col else ""

        mask = pd.Series(False, index=history.index)

        if withdrawal_location_col:
            hloc = history[withdrawal_location_col].astype(str).str.strip().str.lower()
            mask = mask | hloc.str.contains(loc, regex=False, na=False)
            if atm_id_col:
                aid = str(row[atm_id_col]).strip().lower()
                if aid:
                    mask = mask | hloc.str.contains(aid, regex=False, na=False)

        if withdrawal_area_col and area:
            harea = history[withdrawal_area_col].astype(str).str.strip().str.lower()
            mask = mask | harea.str.contains(area, regex=False, na=False)

        return int(mask.sum())

    candidates["historical_withdrawals"] = candidates.apply(
        historical_count, axis=1
    )

    # Mule activity evidence. Prefer city, then area if available.
    mule_city_col = first_existing(mule_df, ["city", "City", "mule_city", "Mule City"])
    mule_area_col = first_existing(mule_df, ["area", "Area", "mule_area", "Mule Area"])

    def mule_activity_count(row):
        if mule_df.empty:
            return 0

        if mule_area_col and atm_area_col:
            area = str(row[atm_area_col]).strip().lower()
            if area:
                m = mule_df[mule_area_col].astype(str).str.strip().str.lower()
                area_count = int(m.str.contains(area, regex=False, na=False).sum())
                if area_count:
                    return area_count

        if mule_city_col:
            m = mule_df[mule_city_col].astype(str).str.strip().str.lower()
            return int(m.str.contains(str(selected_city).strip().lower(),
                                       regex=False, na=False).sum())

        return 0

    candidates["mule_activity"] = candidates.apply(
        mule_activity_count, axis=1
    )

    # Exact scoring structure from the Colab approach:
    # 70% historical withdrawal evidence + 30% mule activity.
    candidates["historical_score"] = minmax_series(
        candidates["historical_withdrawals"]
    ) * 100
    candidates["mule_activity_score"] = minmax_series(
        candidates["mule_activity"]
    ) * 100
    candidates["combined_risk_score"] = (
        0.70 * candidates["historical_score"]
        + 0.30 * candidates["mule_activity_score"]
    )

    candidates["risk"] = candidates["combined_risk_score"].apply(risk_label)
    candidates = candidates.sort_values(
        ["combined_risk_score", "historical_withdrawals", "mule_activity"],
        ascending=False
...
# -------------------- CASH-OUT INTELLIGENCE --------------------
    ...
    c1, c2, c3, c4 = st.columns(4)
    atm_id_col = first_existing(
        atm_df, ["ATM_ID", "atm_id", "ATM ID", "id"]
    )
    atm_loc_col = first_existing(
        atm_df,
        ["ATM_Location", "atm_location", "ATM Location", "location", "Location"]
    ...
    atm_area_col = first_existing(
        atm_df, ["Area", "area", "atm_area", "ATM Area"]
    )

    c1.metric(
        "ATM",
        str(predicted[atm_id_col]) if atm_id_col else "N/A"
    )
    c2.metric("Location", str(predicted[atm_loc_col]))
    c3.metric(
        "Risk Score",
    ...
    if str(predicted["risk"]) == "HIGH":
        st.error(
            ...
            f"{predicted[atm_loc_col]} ({selected_city})"
        )
    ...
    display_cols = []
    if atm_id_col:
        display_cols.append(atm_id_col)
    display_cols.append(atm_loc_col)
    if atm_area_col:
        display_cols.append(atm_area_col)
    ...
    st.write(
        f"**Predicted location:** {predicted[atm_loc_col]}"
    )
    if atm_area_col:
        st.write(
            f"**Area:** {predicted[atm_area_col]}"
        )
    st.write(
