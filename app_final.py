...
# -------------------- CASH-OUT INTELLIGENCE --------------------
    ...
    # Show every ATM reference location (10 records), not unique cities.
    atm_id_col = first_existing(atm_df, ["atm_id", "ATM_ID", "ATM ID", "id"])
    atm_city_col = first_existing(atm_df, ["city", "City", "atm_city", "ATM City"])
    atm_area_col = first_existing(atm_df, ["area", "Area", "atm_area", "ATM Area"])

    if atm_id_col and atm_city_col and atm_area_col:
        atm_choices = atm_df[[atm_id_col, atm_city_col, atm_area_col]].dropna().copy()
        atm_choices[atm_id_col] = atm_choices[atm_id_col].astype(str).str.strip()
        atm_choices[atm_city_col] = atm_choices[atm_city_col].astype(str).str.strip()
        atm_choices[atm_area_col] = atm_choices[atm_area_col].astype(str).str.strip()
        atm_options = [
            f"{row[atm_id_col]} — {row[atm_area_col]} ({row[atm_city_col]})"
            for _, row in atm_choices.iterrows()
        ]
    else:
        st.error("ATM location dataset is missing atm_id, city, or area fields.")
        st.stop()

    active_city = st.session_state.get("active_city")
    default_index = 0
    if active_city:
        for i, (_, row) in enumerate(atm_choices.iterrows()):
            if row[atm_city_col] == active_city:
                default_index = i
                break

    selected_atm_option = st.selectbox(
        "ATM Location",
        atm_options,
        index=default_index,
    )
    selected_atm_index = atm_options.index(selected_atm_option)
    selected_city = str(atm_choices.iloc[selected_atm_index][atm_city_col])
