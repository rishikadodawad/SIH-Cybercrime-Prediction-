...
# -------------------- DATASET OVERVIEW --------------------
    ...
    st.subheader("Banking / Cybercrime Dataset")
    st.write(f"Rows: **{len(bank_df)}**")
    st.write("Columns:")
    st.write(list(bank_df.columns))
    st.dataframe(bank_df.head(20), use_container_width=True)

    st.subheader("Mule Network Dataset")
    st.write(f"Rows: **{len(mule_df)}**")
    st.write("Columns:")
    st.write(list(mule_df.columns))
    st.dataframe(mule_df.head(20), use_container_width=True)

    st.subheader("ATM Location Dataset")
    st.write(f"Rows: **{len(atm_df)}**")
    st.write("Columns:")
    st.write(list(atm_df.columns))
    st.dataframe(atm_df.head(20), use_container_width=True)
